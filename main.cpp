...
#pragma omp parallel for
    ...
    Data_Components time_tf(sub, tf, nMom); // Y_t = mu
    Data_ODE_Observer tfObs6(time_tf); // obs sums over subset of values
    Data_Components time_5(sub, 5.0, nMom); // Y_t = mu
    ...
    State_N c0 = gen_multi_lognorm_init6(); //gen_multi_lognorm_init6();
    for(int i = 0; i < N; i++){
        integrate_adaptive(controlled_stepper, ode6Sys, c0, t0, tf, dt, tfObs6);
        integrate_adaptive(controlled_stepper, ode6Sys, c0, t0, 5.0, dt, t5Obs6);
        integrate_adaptive(controlled_stepper, ode6Sys, c0, t0, 10.0, dt, t10Obs6);
    }
